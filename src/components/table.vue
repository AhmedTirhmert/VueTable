...
<script setup lang="ts">
  ...
  const loadData = async () => {
    ...
    showNEntries();
    calcNumberOfPages(data.results[0].length);
  };
  const calcNumberOfPages = (dataCount) => {
    pagination.numberOfPages = Math.round(dataCount / 10);
  };
  ...
  const nextPage = () => {
    pagination.page <= pagination.numberOfPages ? pagination.page++ : null;
  };
  const prevPage = () => {
    pagination.page > 0 ? pagination.page-- : null;
  };
</script>
